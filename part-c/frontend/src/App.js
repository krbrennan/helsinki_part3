...
const App = () => {
  ...
  const newErrorMessage = (type, name) => {
    ...
    if(type == 'new'){
      setErrorMessage({err: 'new', message:`Added ${name}`})
    } else if(type == 'CREATE NAME ERROR'){
      setErrorMessage({err: 'err', message:`${name} could not be added. Names must be at least 3 characters long`})
      // setErrorMessage('fail', `${name} could not be added. Names must be at least 3 characters long`)
    }
  ...
  const submitName = (event) => {
    ...
    if(nameTaken(newName, persons)) {
      ...
      apiService.create(newPerson)
        ...
        .catch((err) => {
          console.log('ayyyyy ', err)
          newErrorMessage('CREATE NAME ERROR', newPerson.name )
        })
